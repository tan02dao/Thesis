...
#f = open("Sigmaxy_T{}epsilon{}.txt".format(t_init[-1],epsilon),"w+")
...
Sigmaxy = np.zeros(len(epsilon))
Sigmaxx = np.zeros(len(epsilon))
...
for i in range(num_ave):
    ...
    sigxy, sigxx = hall_conduct(eigenvalues, eigenstates, eigenstates_conj, Jx_sparse, Jy_sparse, temperature, mu, epsilon)
    Sigxy += sigxy
    Sigxx += sigxx
